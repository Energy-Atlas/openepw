...
Status: draft for owner review, 2026-09-25. Implement after the Stage 4 MCP contract is accepted; draft evaluation cases can be prepared earlier.
...
| `docs/validation/mcp-stage-5-acceptance.md` | Versioned rubric results, traces and residual agent limitations |
...
- [ ] Record what traces may store: tool names, IDs, statuses, bounded safe summaries, durations and redacted prompt text. Add a test that secrets, file paths and hourly arrays are absent from persisted traces.
...
- [ ] Choose the simpler implementation that meets the rubric. If LangGraph materially improves the evaluated state/recovery flow, keep it as an optional `harness` extra; otherwise retain the direct harness and document why. LangSmith may be used for a redacted opt-in evaluation, never as a required service or default telemetry sink.
...
- [ ] Run every offline fixture through the chosen harness, then a bounded opt-in real-model/client smoke when credentials are available. Record tool traces, rubric scores, failures and corrections. Test that an eligible-but-gapped NOAA retrieval yields an explicit QC warning and no readiness claim.
...
Stage 5 exits with an optional runnable reference agent, a fixed local evaluation suite and redacted traces showing the selected anchor and recovery tasks pass the rubric. Any real-model variability or host-specific behavior is recorded, not hidden. Stage 6 uses this harness and the Stage 4 MCP contract with representative people and a target local client.
