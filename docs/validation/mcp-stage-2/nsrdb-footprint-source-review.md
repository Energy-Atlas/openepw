...
The provider's [TMY API documentation](https://developer.nlr.gov/docs/solar/nsrdb/nsrdb-GOES-tmy-v4-0-0-download/) lists `tdy-2023` as a `names` choice, not an actual 2023 time interval. Stage 1's Ithaca and Phoenix point responses list both selected API products and `tdy-2023`. The bounded coordinate read subsequently found source-grid centers 1.112 km and 0.928 km from those respective points, within the plan's 8 km cross-check. The resulting [acceptance record](nsrdb-footprint-acceptance.md) treats the grid as source-site evidence for this one published selector; it does not infer arbitrary-point API eligibility.
...
NLR attribution is required. The [public AWS registry](https://registry.opendata.aws/nrel-pds-nsrdb/) lists this NLR-managed collection under CC BY 3.0 US; this work retains only local metadata and commits no coordinate inventory. The cross-check has passed. The observed `v4.0.0` path versus `4.0.1` internal model version remains explicit in the manifest and map, so this evidence stays tied to the exact S3 object identity rather than a generalized version claim.
