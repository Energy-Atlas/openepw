...
Date: 2026-09-24. Originating branch: `feature/data-avail`. Scope: the local [multi-product availability map](../../superpowers/plans/2026-09-24-nsrdb-geospatial-availability.md). This evidence is separate from the shared Stage 2 availability service, which was completed on `feature/mcp`. The map generator and tests are tracked; source-coordinate bytes and generated HTML remained ignored in the originating worktree's `.local/mcp-availability/`.
...
The internal `4.0.1` model attribute inside NLR's `v4.0.0` domain remains an explicit provenance discrepancy. The grid map is therefore labeled by both values and is not promoted to positive service eligibility. The completed Stage 2 service has not imported this display mask.
