...
def _verify_local_meta(path: Path, expected_sha256: str) -> None:
    digest = hashlib.sha256()
    try:
        ...
        raise ValueError("local meta checksum unavailable") from error
    if digest.hexdigest() != expected_sha256:
        raise ValueError("local meta checksum mismatch")
...
def acquire_meta(
    ...
    *, probes: tuple[tuple[float, float], ...],
    ...
    if headers is None:
        ...
        _verify_local_meta(destination / "meta.bin", previous.meta_sha256)
