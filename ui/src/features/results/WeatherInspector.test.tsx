...
vi.mock('./WeatherCharts', () => ({ WeatherCharts: ({ heatVariable }: { heatVariable: string }) => <div>heatmap {heatVariable}</div> }))

const artifact = { id: 'a', role: 'weather', path: 'tmyx.epw', media_type: 'application/vnd.energyplus.epw', bytes: 1, sha256: 'x' }
beforeEach(() => useApp.setState({
  artifact,
  inspector: { open: true, manuallyCollapsed: false },
  visualization: {
    calendar: 'gregorian', location: { lat: 42, lon: -76, standard_offset_minutes: -300 },
    monthly: [{ month: 1, year: 2024, expected: 744, source_years: [2008], values: { dry_bulb: { mean: 2, valid: 744 } } }],
    series: { dry_bulb: [2, null] }, timestamps: ['2024-01-01T01:00:00', '2024-01-01T02:00:00'],
    source_years: [2008, 2012], total_rows: 8784, units: { dry_bulb: 'degC' },
    simulation_ready: false, synthetic_chronology: true, warnings: ['Missing irradiance'],
  },
}))
afterEach(cleanup)
...
it('loads another full-artifact variable and remembers manual collapse', () => {
  ...
  expect(run).toHaveBeenCalledWith({ type: 'selectArtifact', artifact, variables: ['dry_bulb', 'liquid_precipitation', 'ghi'] })
