...
export function ProjectPanel() {
  ...
  useEffect(() => {
    if (!baseline || !state.future.baseline || planCurrent) return
    const timer = window.setTimeout(() => run({ type: 'planFuture' }), 300)
    return () => window.clearTimeout(timer)
  }, [baseline, planCurrent, state.future.baseline, state.futureVersion])
      ...
      <section className="control-section baseline-summary">
        ...
        {baseline ? (
          ...
          >
            ...
            <small>{baseline.id.slice(0, 12)} · parsed EPW</small>
          </button>
        ) : (
