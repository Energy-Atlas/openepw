...
| Interactive console chat | Implemented, offline and bounded live tested | Local stdio conversation with automatic plan execution, EPW upload/inspect/save and in-session artifact references; optional LangSmith turn/intent/MCP traces when key exists; existing ignored `.env` read only |
