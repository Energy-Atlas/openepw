...
| Interactive console chat | Implemented, offline and bounded live tested | In-memory request draft, numbered geocoder selection and read-only catalog exploration; automatic plan execution, EPW upload/inspect/save, optional LangSmith traces; existing ignored `.env` read only |
